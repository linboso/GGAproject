...
public class new_MODSTR {
	public static void main(String[] args) throws IOException{
		...
		int K = 3; //�N�Ѳ�����K�s, �ثe�i�H�]����K= 3, 4, 5, 6���i�H���`�B�@
		...
		*/
			for(int trte=0; trte<Testing.length;trte++){ // trte = traning & testing?
				...
				int num=readTECInputData(fileTEC);//Ūtraining�����X��
				int num1=readTECInputData(TestfileTEC);//ŪTESTING���X��
				String[][] inputDataTEC = readTECInputData1(fileTEC, num);//Ūtraining�X�~���Ҧ����
				String[][] inputTestfileTEC = readTECInputData1(TestfileTEC, num1);//Ūtesting�X�~���Ҧ����
				long startTime = System.currentTimeMillis(); // ���o�{�b���ɶ� (�@��) 
				...
				double STOPPP[]={0.0, 0.05, 0.10, 0.15, 0.05, 0.05, 0.10, 0.15, 0.15}; // ���Q�I
				double STOPLOSE[]={0.0, -0.05, -0.10, -0.15, -0.10, -0.15, -0.05, -0.05, -0.10}; // ���l�I
				...
				for(int C=0;C<=8;C++){
					...
					input1=pop1(STOPPP[C],STOPLOSE[C],inputDataTEC, inputTestfileTEC, num,chooseReturnRate,chooseMDD,choosecount,num1,input1);//�[����|��
				}
				for(int C=0;C<=8;C++){
	...
	private static Hashtable<String, Hashtable> pop( double stopPP, double stoplose, String[][] inputDataTEC, String[][] inputTestfileTEC, int num, int chooseReturnRate, int chooseMDD, int choosecount,int num1, Hashtable<String, Hashtable> input) {
		...
		String stopp=stopPP+","+stoplose; // �S��
		...
		while(co!=107){
			...
			if(str.size()==0){
				...
				double MDD=MDD(str,Count);//���t���̤j�Ȳ����l
				double MDD1=MDD1(str,Count);//�H�e��
				...
				if (RankReturnRate[j] < RankReturnRate[j + 1]){
					...
					for(int i=0;i<alre.size();i++){
						...
						if(str1.size()==0){
							...
							double MDD=MDD(str1,Count);//���t���̤j�������l
							double MDD1=MDD1(str1,Count);//���t���̤j�������l-tax
							storeb[i][0]=co-6;
							storeb[i][1]=Count;
	...
	public static int readTECInputData(String fileName) {//Ū�X���X��
		int i=0;
		try{
	...
	public static String[][] readTECInputData1(String fileTEC, int num) {//�Ҧ���Ʀs�_��
		//System.out.println(num);
	...
	//		int numberofStock = inputData.length;
		...
		//	AL.add(weight);
				int count1=0;
				double[] CO=new double[K+1]; //�ӧOweight ������
				String a=weight;
				...
				for(int u=0;u<K+1;u++){
					population.get(""+t).strategyweight.add(( (part/(a.length()-K)) *CO[u])/part); // �ӧO�� %
					//=>> �ӧO������ / part
				}
	...
	static boolean check_K_NonEmptyGroup(chromosome chro, int K) {
		boolean flag = false;
		if(chro.groupPart.size()!=K) flag = true ; //���󪺸ܷ|�^��false,������^��true (me)
		return flag;
	}
...
//			System.out.println("population.size()= "+population.size());
	...
	public static ArrayList<String[]> STR11(String[][] inputDataTEC, int num,int count, double stopPP, double stoplose ) {
		...
		for(int i=0;i<num-1;i++){
			...
			if(!inputDataTEC[i][count].equals("")){
				...
				aa[3]=inputDataTEC[i+1][1];//�j�Ѫ�OPEN PRICEu
				a.add(aa);
			}
		}
	...
	public static ArrayList<String[]> STR(String[][] inputDataTEC, int num,int count) {
		...
		for(int i=0;i<num-1;i++){
			...
			if(!inputDataTEC[i][count].equals("")){
				...
				aa[3]=inputDataTEC[i+1][1];//�j�Ѫ�OPEN PRICE
				a.add(aa);
			}
		}
	...
	private static Hashtable<String, chromosome> calculateFitnessValue(Hashtable<String, chromosome> population,
		...
		for(int i=0; i<population.size();i++){
			...
			possibleStockCombination = getCombination(st, ht.size());//�ثe�u��K=3,4,5,6 ���G�|���T
			double roi=0;
			double risk=0;
	...
	private static double ROI(ArrayList strategyweight, double[][] inputData, String[] StockCombination, int maxCaptial) {
			...
			//	System.out.print("stockNum"+stockNum);
				...
				double c = Double.parseDouble(strategyweight.get(j).toString());//�v��
				double d = a*c*maxCaptial;//Return*�v��
				roi =roi+d;
...
//				}
			...
			//System.out.println("grouping part�@��t");
...
//				System.out.println("chro[1]_Group"+i+": "+chro[1].groupPart.get(""+i).toString());
		...
		//Base chromosome, ��X���J�_�I
		int indexBase = (int)(chro[0].groupPart.size()*Math.random());
		//inserted chromosome, ��X���ΰ_�I�P���I
		...
		//�qinserted chromosome���X���ݴ��Jbase chromosome���s��
...
//			}
	...
	//bug-------------------------------------------------------------------
		//�ھڱ��[�J��group���Ѳ�, �Nbase chromosome���]�t���Ѳ��q�R��
...
//		    }
		...
		//Debug  ���pstArray[j]=0 ;�w�� 0,10,=>10,�Ӥ��O 1, 
		for(Enumeration e = ht.keys();e.hasMoreElements();){
			String st = e.nextElement().toString();
			String[] stArray=ht.get(st).toString().split(",");
...
//		    System.out.println("AA");
		...
		if(aList.size()>chro[0].groupPart.size()){
			...
			for(int i=0; i<removalNumGroup;i++){
				aList = adjustNumberofGroup(aList, -1);//-1���ܭn���group�ƥ�
				//System.out.println("���group�ƥ�");
			}
		}
		...
		if(aList.size()<chro[0].groupPart.size()){
			...
			for(int i=0; i<addNumGroup;i++){
				aList = adjustNumberofGroup(aList, 1);//1���ܭn�W�[group�ƥ�
				//System.out.println("�W�[group�ƥ�");
			}     
...
//			}
	 ...
	 //   System.out.println("crossoverChro[0].stockPortfolioSize="+cutPoint+" "+cutPoint1);
	   //�ھ�cutpoint�洫c1�Pc2��strategyweight part
	...
	//	System.out.println("After="+oo.length());
		...
		for(int i=0; i<mutatioGene; i++){
			...
			//	System.out.println("firstwhichPosition1 "+whichPosition1+" "+whichPosition);	
				...
				chro.strategyweight = aL; //��aL�Ȧs�^chro
...
//						st = st.replace(stArr[stArr.length-1]+",", "");
	...
	private static double printAChromosome(Hashtable<String, chromosome> population, int i, Hashtable<String, Hashtable> input, BufferedWriter finalr, int maxCaptial,double stoppp,double stoplose) throws NumberFormatException, IOException {
		...
		ArrayList<String[]> listnum = new ArrayList<String[]>();//��C�Ӹ��,ROI�M���ǵ���
	...
	//	System.out.println("����"+possibleStockCombination.size()); 
...
/*			 avgTime =0;
		...
		finalr.newLine();
		System.out.printf("�����@�յ������S1: "+ population.get(""+i).fitness[1]);
		finalr.write("�����@�յ������S1: "+ population.get(""+i).fitness[1]);
		return population.get(""+i).fitness[1];
	}
	private static boolean C(Hashtable<String, chromosome> population, int i) {//�p�Ggroup���@�Ӥ@�ժ��ܦ^��FALSE
		// TODO Auto-generated method stub
		int a=0;
		Hashtable htt = (Hashtable) population.get(""+i).groupPart;
...
//��̤j�̤p��
		...
		finalr.newLine();
//					System.out.println("����"+possibleStockCombination.size()+"  "+ss/listnum.size()+"R"+sumROI/possibleStockCombination.size()+" C  "+sumcap/possibleStockCombination.size());
...
//					c=Double.parseDouble((population.get(""+i).strategyweight.get(1).toString()))*maxCaptial;
		...
		if(Double.valueOf(mon1)!=0){
			aa[1]=Double.valueOf(ro1)/Double.valueOf(mon1);//�̤j
		}
