...
class Dask_TI2Signal():
    ...
    def ProduceSignal(self):
        ...
        if __name__ == "__main__":
            ...
            TI_Format = dd.read_json('./Case/TIformat.json', encoding='utf-8')
        else:
            try:
                with open('./PreProCessing/Case/TIformat.json', 'r', encoding="utf-8") as f:
                    TI_Format = dd.read_json(f)
            except:
        ...
        for c in Combinaion:
            ...
            data = dd.concat([data, signal], axis=1)

        
        data.to_csv(f"{self.path}/DaskSignal.csv")
    
        # data.to_json(f"{self.path}/DaskSignal.json", orient='columns')
